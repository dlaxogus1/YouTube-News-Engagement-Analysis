...
API_KEY <- "API_KEY"
HANDLE <- "newskbs"     # https://www.youtube.com/@newskbs/videos
MIN_VIEWS <- 500000
TARGET_VIDEOS <- 2500

# @newskbs 핸들로 업로드 플레이리스트 ID 찾기
res <- GET(
...
channel_data <- fromJSON(content(res, "text", encoding = "UTF-8"), flatten = TRUE)
...
while(length(video_ids) < TARGET_VIDEOS){
  ...
  video_ids <- c(video_ids, data$items$contentDetails.videoId)
...
for(i in seq(1, length(video_ids), by = 50)){
  ...
  info <- fromJSON(content(res3, "text", encoding = "UTF-8"), flatten = TRUE)
...
# 링크 추가
result <- result %>%
  mutate(url = paste0("https://www.youtube.com/watch?v=", videoId))
